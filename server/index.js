...
app.get('/api/members', authenticateToken, requirePermission('members:read'), async (req, res) => {
...
app.get('/api/members/:id', authenticateToken, requirePermission('members:read'), async (req, res) => {
  const { id } = req.params;
  try {
...
app.post('/api/members', authenticateToken, requirePermission('members:create'), async (req, res) => {
  const { id, firstName, lastName, email, telephone, address, city, state, zip, familyId } = req.body;
  
  // High-priority validation check
  const validation = validateMember({ firstName, lastName, email, telephone, state, zip });
  ...
  try {
    const result = await pool.query(
      ...
      [memberId, firstName, lastName, email, telephone, address, city, state, zip, familyId]
    );
...
app.put('/api/members/:id', authenticateToken, requirePermission('members:update'), async (req, res) => {
  const { id } = req.params;
  const { firstName, lastName, email, telephone, address, city, state, zip, familyId } = req.body;
  ...
  try {
    const result = await pool.query(
      ...
      [firstName, lastName, email, telephone, address, city, state, zip, familyId, id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Member not found' });
    }
...
app.delete('/api/members/:id', authenticateToken, requirePermission('members:delete'), async (req, res) => {
  const { id } = req.params;
  try {
...
app.get('/api/donations/:id', authenticateToken, requirePermission('donations:read'), async (req, res) => {
  const { id } = req.params;
  try {
...
app.post('/api/donations', authenticateToken, requirePermission('donations:create'), async (req, res) => {
...
app.put('/api/donations/:id', authenticateToken, requirePermission('donations:update'), async (req, res) => {
  const { id } = req.params;
...
app.delete('/api/donations/:id', authenticateToken, requirePermission('donations:delete'), async (req, res) => {
  const { id } = req.params;
  try {
...
app.get('/api/reports/statements', authenticateToken, requirePermission('reports:export'), async (req, res) => {
...
app.get('/api/reports/export', authenticateToken, requirePermission('reports:export'), async (req, res) => {
  const { year } = req.query;
  if (!year) return res.status(400).json({ error: 'Year is required' });
...
app.get('/api/reports/missing-emails', authenticateToken, requirePermission('reports:read'), async (req, res) => {
  try {
    const result = await pool.query(`
...
app.get('/api/reports/new-donors', authenticateToken, requirePermission('reports:read'), async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
...
app.get('/api/reports/fund-distribution', authenticateToken, requirePermission('reports:read'), async (req, res) => {
  const { year } = req.query;
  if (!year) return res.status(400).json({ error: 'Year is required' });
...
app.get('/api/reports/quarterly-progress', authenticateToken, requirePermission('reports:read'), async (req, res) => {
  const { year } = req.query;
  if (!year) return res.status(400).json({ error: 'Year is required' });
...
app.get('/api/reports/trend-analysis', authenticateToken, requirePermission('reports:read'), async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
        EXTRACT(YEAR FROM donation_date)::int as year,
...
app.post('/api/users', authenticateToken, requirePermission('users:write'), async (req, res) => {
  ...
  try {
    ...
    console.log(`[AUDIT] User created: ${username} with role ${requestedRole} by ${req.user.username}`);
...
app.delete('/api/users/:id', authenticateToken, requirePermission('users:delete'), async (req, res) => {
  ...
  try {
    ...
    console.log(`[AUDIT] User ${targetUser.rows[0].username} (${id}) deleted by ${req.user.username}`);
...
app.post('/api/users/:id/unlock', authenticateToken, requirePermission('users:write'), async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    console.log(`[AUDIT] User ${result.rows[0].username} unlocked by ${req.user.username}`);
...
app.post('/api/users/:id/reset-password', authenticateToken, requirePermission('users:write'), async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (!canManageRole(req.user.role, targetUser.rows[0].role)) {
      return res.status(403).json({ 
        error: 'Cannot reset password for user with role equal to or higher than yours',
        code: 'INSUFFICIENT_PRIVILEGE'
      });
    }
    ...
    console.log(`[AUDIT] Password reset for ${targetUser.rows[0].username} by ${req.user.username}`);
...
app.get('/api/roles', authenticateToken, requirePermission('users:read'), async (req, res) => {
  ...
  const assignableRoles = Object.entries(ROLE_LEVELS)
    .filter(([role, level]) => level < userLevel || req.user.role === 'super_admin')
    .map(([role, level]) => ({
      name: role,
      level,
